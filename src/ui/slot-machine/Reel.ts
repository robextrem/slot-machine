...
export default class Reel extends PIXI.Container {
  ...
  constructor(width: number, height: number, index: number) {
    ...
    for (let i = 0; i < this.numberOfSlots + 2; i++) {
      const slot = new Slot(
        this.reelWidth,
        this.reelHeight / this.numberOfSlots,
        i,
      )
  ...
  public spin = (duration: number, delay: number, cb: () => void): void => {
    ...
    const blockSize =
      import.meta.env.VITE_APP_HEIGHT / (import.meta.env.VITE_APP_NUM_SLOTS + 1)
        ...
        .to(this, {
          ...
          onUpdate: () => {
            ...
            if (slot.y >= this.container.height) {
              ...
              slot.swap(
                tween.totalTime() >= duration - 1
                  ? this.symbols[slot.index]
                  : null,
              )
  ...
  private initialPosition(): void {
    const blockSize = Math.ceil(
      this.reelHeight / import.meta.env.VITE_APP_NUM_SLOTS,
    )
